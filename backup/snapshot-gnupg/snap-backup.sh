...
tar czf - /mnt/snap_mount | sudo gpg --batch --yes --encrypt --recipient nginxwebserver@gmail.com --trust-model always --output /free/backup/backup.tar.gz.gpg
umount /mnt/snap_mount
# 2. Rsync the ENCRYPTED file only
rsync -az /free/backup/backup.tar.gz.gpg hilal@192.168.100.214:~/backup_folder
